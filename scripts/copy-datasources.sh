...
SOURCE_DIR=~/Workspaces/core-platform-config/grafana-datasources/datasources
log "Source directory: ${SOURCE_DIR}"

if [ ! -d "${SOURCE_DIR}" ]; then
    exit_with_error "Source directory does not exist."
fi

cp -r "${SOURCE_DIR}"/{alto,arcus,core,eleos,kraken,kronos,lakitu,loki,nova,ovp-teams-federate,pulsar}* "${DATASOURCE_DIR}"
if [ $? -ne 0 ]; then
